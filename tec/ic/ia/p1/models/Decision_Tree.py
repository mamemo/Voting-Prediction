...
class DecisionTree(Model):
    def __init__(self, samples_train, samples_test, prefix, pruning_threshold):
        ...
        self.votos = [['ACCESIBILIDAD SIN EXCLUSION', 'ACCION CIUDADANA', 'ALIANZA DEMOCRATA CRISTIANA', 'DE LOS TRABAJADORES', 'FRENTE AMPLIO', 'INTEGRACION NACIONAL', 'LIBERACION NACIONAL', 'MOVIMIENTO LIBERTARIO',
                       'NUEVA GENERACION', 'RENOVACION COSTARRICENSE', 'REPUBLICANO SOCIAL CRISTIANO', 'RESTAURACION NACIONAL', 'UNIDAD SOCIAL CRISTIANA', 'NULOS', 'BLANCOS'], ['ACCION CIUDADANA', 'RESTAURACION NACIONAL', 'NULOS', 'BLANCOS']]
        self.pruning_threshold = pruning_threshold
        # Variables used on Gain()
        self.entropia_general = 0
        self.lista_atributos = []
        self.valores_atributos = []
        self.valores_answers = []
        self.lista_answers = []
        # Use for importance function
        self.total = []
        # Examples amount
        self.N = 0
        # Stores the childs of every attribute
    ...
    # Function responsible of getting the attribute gain
    ...
    # Go through the data list to obtain the amount of values at vk.
    def sacar_datos(self, atributo, datos):
        for i in datos:
            if (self.lista_atributos.count(self.samples_train[0][i][atributo]) == 0):
                ...
                self.valores_atributos[self.lista_atributos.index(
                    self.samples_train[0][i][atributo])] += 1
            if (self.lista_answers.count(self.samples_train[1][i]) == 0):
                ...
                self.valores_answers[self.lista_answers.index(
                    self.samples_train[1][i])] += 1
            self.total[self.lista_atributos.index(
                self.samples_train[0][i][atributo])][self.lista_answers.index(self.samples_train[1][i])] += 1

    # Gets the attribute entropy.
    ...
    # Gets the attribute's child entropy and substract it to the attribute entropy.
    def gain_total(self, atributo, datos):
        ...
        for i in range(len(self.valores_atributos)):
            ...
            for j in range(len(self.valores_answers)):
                if(self.total[i][j] != 0):
                    resultado_parcial += (self.total[i][j]/self.valores_atributos[i])*log(
                        self.total[i][j]/self.valores_atributos[i], 2)
            resultado += (self.valores_atributos[i] /
                          self.N) * (-1) * (resultado_parcial)
        return(self.entropia_general - resultado)

    # Establishes the numeric values ranges.
    ...
    # Function in charge of creating the decision tree.
    ...
    def decision_tree_learning(self, examples, attributes, parent_examples):
        ...
        if (examples == []):
            ...
            ejemplos = [[] for i in range(
                len(self.valores_atributos_oficiales[tree.atributo]))]
            v = []
            for i in examples:
                v = self.valores_atributos_oficiales[self.atributos_oficiales.index(
                    tree.atributo)]
    ...
    # Gets the most common value from data.
        ...
        for i in examples:
            if(self.samples_train[0][i][tree.atributo] not in attributes):
                ...
                values[attributes.index(
                    self.samples_train[0][i][tree.atributo])] += [i]
        cant_hojas = 0
        for k in range(len(tree.hijos)):
            if (type(tree.hijos[k]) != type("")):
                #atributo = tree.hijos[k].atributo
                tree.hijos[k] = self.pruning_tree(
                    values[attributes.index(tree.condicion[k])], tree.hijos[k])
                if (type(tree.hijos[k]) == type("")):
                    cant_hojas += 1
    ...
    def total_desviation(self, attributes, examples):
        ...
        for i in range(len(outputs_x_attributes)):
            ...
            for j in range(len(outputs_x_attributes[i])):
                ...
                partial_result += ((outputs_x_attributes[i]
                                    [j] - true_irrelevance)**2)/true_irrelevance
    ...
    def validar_datos(self, data):
        ...
        for i in range(len(data[0])):
            ...
            while (hijo == False):
                if(arbol.condicion.count(data[0][i][arbol.atributo]) != 0):
                    arbol = arbol.hijos[arbol.condicion.index(
                        data[0][i][arbol.atributo])]
                    if(type(arbol) == type("")):
                        hijo = True
                else:
                    print(
                        "Atributo: \"", data[0][i][arbol.atributo], "\" no se encuentra en arbol.")
                    break
        ...
        print("Aciertos: ", count, " De: ", len(
            data[0]), " Accuracy: ", (count/len(data[0])*100), "%")
    ...
    def execute(self):
        ...
        if(type(self.main_tree) == type("")):
            ...
            print("-> Training Set")
            print("Aciertos: ", self.samples_train[1].count(self.main_tree), " De: ", len(
                self.samples_train[0]), " Accuracy: ", (self.samples_train[1].count(self.main_tree)/len(self.samples_train[0]))*100, "%")
            print("-> Test Set")
            print("Aciertos: ", self.samples_test[1].count(self.main_tree), " De: ", len(
                self.samples_test[0]), " Accuracy: ", (self.samples_test[1].count(self.main_tree)/len(self.samples_test[0]))*100, "%")
