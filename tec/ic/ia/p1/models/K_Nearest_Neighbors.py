...
class KNearestNeighbors(Model):
    ...
    # Recursive function that looks for the k closest points to the one being classified.
    # Input: The root of the kd tree, the point being classified and the current level of the tree.
    ...
    def kdtree_closest_point(self, root, point, depth=0):
        ...
        if abs(point[axis] - root['sample'][axis]
            ...
            best_right = self.kdtree_closest_point(
                opposite_branch, point, depth + 1)
    ...
    # Function that its called to execute the model. It creates the tree, and tests it using the training and testing set.
    # Input: -
    ...
    def execute(self):
        ...
        print("Error con train set: ", n_results / len(self.samples_train[0]))
        ...
        for i in range(0, len(self.samples_test[0])):
            ...
            if sample_result == pred_result:
                ...
                n_results += 1
            pred_votes.append(pred_result)
        print("Error con test set: ", n_results / len(self.samples_test[0]))
        return pred_votes
